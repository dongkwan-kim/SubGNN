...
class SyntheticGraph():
    ...
    def generate_and_add_subgraphs(self, **kwargs):
        ...
        if self.subgraph_type == 'random':
            ...
            subgraphs = self._get_subgraphs_by_k_hops(n_subgraphs, n_nodes_in_subgraph, n_connected_components,
                                                      **kwargs)
            ...
            if desired_property == 'coreness':
                ...
                subgraphs = self._get_subgraphs_by_planting(n_subgraphs, n_nodes_in_subgraph, n_connected_components,
                                                            **kwargs)
        else:
    ...
    def _get_subgraphs_by_k_hops(self, n_subgraphs, n_nodes_in_subgraph, n_connected_components, **kwargs):
        ...
        for s in range(n_subgraphs):
            ...
            candidate_nodes = [node for node in n_hops_paths if
                               self.is_k_hops_from_all_cc(node, all_cc_start_nodes, k_hops) and node not in seen_nodes]

            if len(candidate_nodes) == 0: candidate_nodes = [node for node, length in n_hops_paths.items() if
                                                             length == max(n_hops_paths.values())]
            ...
            for c in range(n_connected_components - 1):
                ...
                cc_node_ids, cc_root_node = self.staple_component_to_graph(n_nodes_in_subgraph, new_graph_root_node,
                                                                           **kwargs)
                curr_subgraph.extend(cc_node_ids)
                seen_nodes.extend(cc_node_ids)
                all_cc_start_nodes.append(cc_root_node)  # keep track of start nodes across CCs
    ...
    def _get_subgraphs_by_coreness(self, n_subgraphs, n_nodes_in_subgraph, n_connected_components, remove_edges=False,
                                   **kwargs):
        """
        ...
        for k in nodes_with_core_number:
            ...
            for s in range(n_subgraphs):
                ...
                for c in range(n_connected_components):
                    ...
                    joined_graph = nx.compose(self.graph,
                                              con_component)  # NOTE: attributes from subgraph take precedent over attributes from self.graph
                    self.graph = joined_graph.copy()
        ...
        for s in range(n_subgraphs):
            ...
            for start_node in start_nodes:
                ...
                assert nx.is_connected(
                    self.graph.subgraph(nodes))  # check to see if selected nodes represent a conencted component
        ...
        for s in range(n_subgraphs):
            ...
            for c in range(n_connected_components):
                ...
                if c > 0 and k_hops != -1:
                    ...
                    candidate_nodes = [cand for cand in candidate_nodes if
                                       self.is_k_hops_from_all_cc(cand, all_cc_start_nodes, k_hops)]
                    ...
                    k_core_dict = nx.core_number(self.graph)
                    nodes_with_core_number = [node for node, core_num in k_core_dict.items() if
                                              core_num == k_core_to_sample]
    ...
    def _get_subgraphs_by_planting(self, n_subgraphs, n_nodes_in_subgraph, n_connected_components, remove_edges=False,
                                   **kwargs):
        """
    ...
    def _relabel_nodes(self, subgraphs, **kwargs):
        ...
        print("Original graph: %d, Largest cc: %d, Removed nodes: %d" % (
