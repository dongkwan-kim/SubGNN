...
def get_hyperparams_optuna(run_config, trial):
    ...
    hyp_optuna = {k: get_optuna_suggest(run_config["hyperparams_optuna"][k], k, trial) for k in
                  dict(run_config["hyperparams_optuna"]).keys()}
...
def build_model(run_config, trial=None):
    ...
    model = md.SubGNN(hyperparameters, run_config["graph_path"], \
                      ...
                      run_config["similarities_path"], run_config["shortest_paths_path"],
                      run_config['degree_sequence_path'], run_config['ego_graph_path'])
...
def build_trainer(run_config, hyperparameters, trial=None):
    ...
    if not os.path.exists(lgdir):
        os.makedirs(lgdir)
    logger = TensorBoardLogger(run_config['tb']['dir_full'], name=run_config['tb']['name'],
    ...
    if run_config["optuna"]['pruning']:
        trainer_kwargs['early_stop_callback'] = PyTorchLightningPruningCallback(trial, monitor=run_config['optuna'][
            'monitor_metric'])
...
def main():
    ...
    study.optimize(lambda trial: train_model(run_config, trial), n_trials=run_config['optuna']['opt_n_trials'],
