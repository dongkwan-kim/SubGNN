...
def triangular_random_walk(hparams, networkx_graph, anchor_patch_subgraph, walk_len, in_border_nodes, all_valid_nodes,
                           inside):
    """
    ...
    if inside:
        ...
        curr_node = np.random.choice(neighbor_nodes) if len(
            neighbor_nodes) > 0 else config.PAD_VALUE  # we're using the PAD_VALUE as a sentinel that the first node has no neighbors
    ...
    for k in range(walk_len - 2):
        ...
        triangular_neighbors, non_triangular_neighbors = get_neighbors(networkx_graph, anchor_patch_subgraph,
                                                                       all_valid_nodes, prev_node, curr_node,
                                                                       inside=inside)
...
def perform_random_walks(hparams, networkx_graph, anchor_patch_ids, inside):
    ...
    for anchor_patch in anchor_patch_ids:
        ...
        if curr_anchor_patch.shape[0] == 0:
            all_patch_walks.append(
                torch.zeros((hparams['n_triangular_walks'], hparams['random_walk_len']), dtype=torch.long).fill_(
                    config.PAD_VALUE))
        else:

            anchor_patch_subgraph = networkx_graph.subgraph(
                curr_anchor_patch.numpy())  # create a networkx graph from the anchor patch
            if not inside:
                # get nodes in subgraph that have an edge to a node not in the subgraph & all of the nodes that are not in the subgraph
                in_border_nodes, non_subgraph_nodes = subgraph_utils.get_border_nodes(networkx_graph,
            ...
            for w in range(hparams['n_triangular_walks']):
                walk = triangular_random_walk(hparams, networkx_graph, anchor_patch_subgraph,
                                              hparams['random_walk_len'], in_border_nodes, all_valid_nodes,
                                              inside=inside)
    ...
    all_patch_walks = torch.stack(all_patch_walks).view(n_sampled_patches, hparams['n_triangular_walks'],
...
def sample_structure_anchor_patches(hparams, networkx_graph, device, max_sim_epochs):
    ...
    for i, node in enumerate(start_nodes):
        ...
        if hparams['structure_patch_type'] == 'ego_graph':
            ...
            subgraph = triangular_random_walk(hparams, networkx_graph, networkx_graph, hparams['sample_walk_len'], None,
                                              None, True)
        else:
    ...
    for s in all_patches:
        ...
        padded_all_patches.append(
            torch.cat([torch.LongTensor(s), torch.LongTensor((fill_len)).fill_(config.PAD_VALUE)]))
...
def init_anchors_neighborhood(split, hparams, networkx_graph, device, train_cc_ids, val_cc_ids, test_cc_ids,
    ...
    for dataset_name, dataset, border_set in zip(dataset_names, datasets, border_sets):
        for n in range(hparams['n_layers']):
            anchors_int_neigh[dataset_name][n] = sample_neighborhood_anchor_patch(hparams, networkx_graph, dataset,
                                                                                  border_set, sample_inside=True)
            anchors_border_neigh[dataset_name][n] = sample_neighborhood_anchor_patch(hparams, networkx_graph, dataset,
                                                                                     border_set, sample_inside=False)
...
def init_anchors_structure(hparams, structure_anchors, int_structure_anchor_rw, bor_structure_anchor_rw):
    ...
    for n in range(hparams['n_layers']):
        indices = list(
            np.random.choice(range(structure_anchors.shape[0]), hparams['n_anchor_patches_structure'], replace=True))
        anchors_struc[n] = (structure_anchors[indices, :], indices, int_structure_anchor_rw[indices, :, :],
...
def get_anchor_patches(dataset_type, hparams, networkx_graph, node_matrix, \
    ...
    if channel == 'neighborhood':
        ...
        anchor_patches, indices, int_anchor_rw, bor_anchor_rw = anchors_structure[
            layer_num]  # (n_anchor_patches_sampled, max_length_anchor_patch)
        ...
        anchor_embeds = aggregate_structure_anchor_patch(hparams, networkx_graph, lstm, node_matrix, anchor_patches,
                                                         anchor_rw, inside=inside, device=cc_ids.device)
...
def aggregate_structure_anchor_patch(hparams, networkx_graph, lstm, node_matrix, anchor_patch_ids, all_patch_walks,
                                     inside, device):
    """
    ...
    walk_embeds_reshaped = walk_embeds.view(n_sampled_patches * hparams['n_triangular_walks'],
                                            hparams['random_walk_len'], hparams['node_embed_size'])
